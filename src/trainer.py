...
class Trainer:
    ...
    def train_component(self, component: nn.Module, optimizer: torch.optim.Optimizer, steps_per_epoch: int,  max_grad_norm: Optional[float],  lr_scheduler= None, **kwargs_loss: Any) -> Dict[str, float]:
        ...
        for _, (imgs, instr, joints, states_tensor, next_imgs, index) in enumerate(tqdm(self.train_dataset, desc="Training", ncols=100)):
            """batch['observation'] is supposed to be channels first and in [0, 1]"""
            B, F, H, W, C = imgs.shape
            imgs = imgs.contiguous().view(B*F, H, W, C).float()
            next_imgs = next_imgs.contiguous().view(B*F, H, W, C).float()
            _, _, V = joints.shape
            _, _, V2 = states_tensor.shape
            joints = joints.contiguous().view(-1, V).float()
            states_tensor = states_tensor.contiguous().view(-1, V2).float()

            imgs = imgs.permute(0, 3, 1, 2).unsqueeze(dim=1) # 'b f c h w'
    ...
    def eval_component(self, component: nn.Module, **kwargs_loss: Any) -> Dict[str, float]:
        ...
        for _, (imgs, instr, joints, states_tensor, next_imgs, index) in enumerate(tqdm(self.test_dataset, desc="Testing", ncols=100)):
            """batch['observation'] is supposed to be channels first and in [0, 1]"""
            B, F, H, W, C = imgs.shape
            imgs = imgs.contiguous().view(B*F, H, W, C).float()
            next_imgs = next_imgs.contiguous().view(B*F, H, W, C).float()
            ...
            imgs = imgs.permute(0, 3, 1, 2).unsqueeze(dim=1)
            next_imgs = next_imgs.permute(0, 3, 1, 2).unsqueeze(dim=1)
            instructions = [] 
            for i in instr:
                instructions += [i] * F
