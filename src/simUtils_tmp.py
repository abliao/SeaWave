    ...
    def moveHand(self,x=0,y=0,z=0,handSide='Right',method='diff',gap=0.3,keep_rpy=None):
        ...
        if gap is None:
            ...
            for index,(nx,ny,nz) in enumerate(np.linspace([ox,oy,oz],[x,y,z],k+1)[1:]):
                ...
                self.setEndPointPosition(action)
                time.sleep(0.01)
                if keep_rpy is not None and (index+1)%(int(3/gap) if int(3/gap)>0 else 1)==0:
        ...
        while True:
            if time.time()-st>1:
                logging.info(f'hand hasn\'t arrive while time limited, diff = {np.linalg.norm(np.array(self.getSensorsData(handSide)[0])-np.array([x,y,z]))}')
                break
            if np.linalg.norm(np.array(self.getSensorsData(handSide)[0])-np.array([x,y,z]))<0.1:
                break
        if keep_rpy is not None:
            self.set_world_rpy(keep_rpy,handSide=handSide)
            time.sleep(0.05)
...
class SimAction(Sim):
    ...
    def closeTargetObj(self,obj_id,handSide='Right',gap=1):
        paths = []
        if handSide=='Right':
            obj_loc=np.array(self.findObj(id=obj_id)['location'])
            obj_loc[0]+=4
            ...
            sensor = self.getSensorsData(handSide='All',type='full')
            middle = np.array(sensor[-10]['data'])
            wrist = self.getSensorsData(handSide=handSide)[0]
            new_paths=getA2B(wrist,obj_loc-(middle-wrist),gap=gap)
            print('wrist',wrist)
            print('obj_loc',obj_loc)
            print('middle',middle)
            for path in new_paths:
                path = np.concatenate((np.array([0,0,0]),path,np.array([0,0])))
                paths.append(path)
            obj_loc=np.array(self.findObj(id=obj_id)['location'])
            obj_loc[0]-=4+2
            obj_loc[1]-=2-1
            ...
            new_paths=getA2B(wrist,obj_loc-(middle-wrist),gap=gap)
            for path in new_paths:
                path = np.concatenate((path,np.array([0,0,0]),np.array([0,0])))
                paths.append(path)
    ...
    def graspTargetObj(self,obj_id,handSide='Right',gap=1,distance=10):
        paths=self.closeTargetObj(obj_id=obj_id,handSide=handSide,gap=gap)
        if handSide=='Left':
            action = list(paths[-1][:3]) + [0]*3+[1,0]
        else:
            action = [0]*3+list(paths[-1][3:6])+[0,1]
    ...
    def placeTargetObj(self,obj_id,handSide='Right',gap=1):
        paths = []
        if handSide=='Right':
            obj_loc=np.array(self.findObj(id=obj_id)['location'])
            ...
            new_paths=getA2B(wrist,obj_loc-(middle-wrist),gap=gap)
            for path in new_paths:
                path = np.concatenate((np.array([0,0,0]),path,np.array([0,1])))
                paths.append(path)
        elif handSide=='Left':
            obj_loc=np.array(self.findObj(id=obj_id)['location'])
            obj_loc[2] = self.desk_height+self.registry_objs[obj_id][1]+3
            ...
            new_paths=getA2B(wrist,obj_loc-(middle-wrist),gap=gap)
            for path in new_paths:
                path = np.concatenate((path,np.array([0,0,0]),np.array([1,0])))
                paths.append(path)
        return paths
...
def getA2B(A,B,gap=1):
    A = np.array(A)
    B = np.array(B)
    paths = [A]
    while np.linalg.norm(B-A)>gap:
        vector = B-A
        unit_vector = vector / np.linalg.norm(vector)
        new_position = A + unit_vector * gap
