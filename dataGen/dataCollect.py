...
import os
output_path='/data2/liangxiwen/zkd/datasets/dataGen/1_objs'
...
for epoch in range(10):
    ...
    for index,data in tqdm(enumerate(shuffled_list)):
        ...
        for action in sim.graspTargetObj(obj_id=1,handSide=handSide,lift_h=20,gap=0.3,return_action=True):
            actions.append(np.array(action))
            time.sleep(0.05)
            images.append(sim.getImage())
            states.append(sim.getState())
        for index,(img,state,action) in enumerate(zip(images,states,actions)):
            each_frame={}
            if last_action[-1]!=action[-1]:
                each_frame['img']=Resize(last_img)
                each_frame['state']=last_state
                each_frame['action']=last_action.copy()
                each_frame['after_state']=states[index-1]
                offline_data['trajectory'].append(each_frame)
                last_action[:6]=0
                last_state=states[index-1]
                last_img=images[index-1]
                dis=0
