...
class Feeder(Dataset):
    ...
    def read_data(self,path):
        ...
        for i in datas:
            data=[]
            for j in i:
                numbers=list(map(float, j.split(',')))
                data.append(numbers)
            df.append(data)
        return df
    ...
    def action_tokenization(self, action):
        ...
        bin_target=action_numpy/(self.joint_boundary[:,1]-self.joint_boundary[:,0])
    ...
    def __getitem__(self, index):
        ...
        dx,dy,dz=-sample[0][0],-sample[0][1],sample[0][2]-10
        ...
        for joint in joints: 
            ...
            states[-1][3:3+21]/=(self.joint_boundary[:,1][:21]-self.joint_boundary[:,0][:21])
            states[-1][3+21:3+21+3]/=100
        ...
        if frame_num < self.sample_frame:
            ...
            states = [states[0]] * (self.sample_frame - frame_num) + states
        ...
        # print(joints_tok[0])
        return imgs_tensor, instr, joints_tok, states_tensor, index